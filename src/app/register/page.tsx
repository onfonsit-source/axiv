...
const timeoutId = setTimeout(() => controller.abort(), 180000); // 3분 타임아웃
  ...
  // 웹서비스 분석 (Python 서버 없이 YouTube API + Tavily)
  const handleAnalyzeWeb = async () => {
    ...
    try {
      ...
      // 상호명을 찾지 못한 경우 → 수동 입력 모달
      if (data.needManualInput || !data.places?.length) {
        setLoading(false);
        showInputModal(
          '장소명 입력',
          '영상에서 상호명을 찾지 못했습니다. 매장 이름을 직접 입력해주세요.',
          async (manualName: string) => {
            ...
            try {
              // 수동 입력된 상호명으로 재검색 요청
              const reResponse = await fetch('/api/analyze-web', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ url, forcePlaceName: manualName }),
              });
              const reData = await reResponse.json();
              if (!reResponse.ok || reData.error) throw new Error(reData.error || '재검색 실패');
              if (!reData.places?.length) {
                showToast('검색 결과가 없습니다.', 'error');
                return;
              }
              setResult(reData);
              fetchCoupangByPlace(reData.places);
              showToast('분석이 완료되었습니다.', 'success');
            } catch (e: any) {
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 md:p-6 shadow-xl border border-gray-100 dark:border-slate-800 mb-10">
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              disabled={loading || !url}
              className="w-full py-3.5 bg-blue-500 hover:bg-blue-600 disabled:bg-slate-200 dark:disabled:bg-slate-800 text-white font-black rounded-2xl transition-all shadow-lg flex items-center justify-center gap-2 active:scale-95 text-sm"
            >
              {loading ? (
                <Loader2 className="w-5 h-5 animate-spin" />
              ) : (
